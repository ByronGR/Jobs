...
export async function signOutCandidate() {
  localStorage.removeItem('nearworkCandidate');
  await signOut(auth);
}
...
export async function submitApplication(applicationData) {
  ...
  let candSnap = await withTimeout(getDocs(query(collection(db,'candidates'), where('email','==',email))), 'candidate lookup');
  ...
  const candidateProfile = {
    ...
    authUid: auth.currentUser?.uid || applicationData.authUid || null,
  ...
  try {
    await withTimeout(addDoc(collection(db,'audit_logs'),{action:'candidate_applied',entity:'candidate',entityId:candId,openingCode,timestamp:now,source:'jobs.nearwork.co',detail:(candSnap.empty?'New candidate':'Updated')+' — applied to '+openingCode}), 'audit log', 6000);
  } catch(e) {
